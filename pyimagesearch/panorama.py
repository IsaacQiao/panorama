...
class Stitcher:
	...
	def stitch(self, images, ratio=0.75, reprojThresh=8,				# reprojThresh -> big,more 
		...
		if M is None:													# list of keypoint, matches
			return None													# homography matrix derived from the RANSAC algorithm, H

		# otherwise, apply a perspective warp to stitch the images		# status , a list of indexes to indicate which keypoints 
		# together														# in matches  were successfully spatially verified using RANSAC.
		(matches, H, status) = M 										
		result = cv2.warpPerspective(imageA, H,							# imageA --- image we want to warp ; H -- transformation matrix 3*3
			(imageA.shape[1] + imageB.shape[1], imageA.shape[0]))		# shape out, (x,y)
